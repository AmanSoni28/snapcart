...
function UserOrderCard({ order }: { order: IOrder }) {
  ...
  useEffect((): any => {
    //socket setup for real time order status update
  ...
  return (
    <motion.div
      ...
      className="bg-white rounded-2xl border border-gray-100 shadow-md hover:shadow-lg transition-all duration-300 overflow-hidden mt-5"
    >
      ...
      <div className="p-5 space-y-4">
        ...
        {order.assignedDeliveryBoy && status!=="delivered" && (
          <>
            ...
            <button className="w-full flex items-center justify-center gap-2 bg-green-600 text-white font-semibold py-4 px-2 rounded-xl shadow hover:bg-green-700 transition" onClick={()=>router.push(`/user/track-order/${order._id?.toString()}`)}>
              <Truck size={18} />
              Track Your Order
            </button>
          </>
