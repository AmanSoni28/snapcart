...
//   console.log(user);
  ...
  const [searchBaxOpen,setSearchBarOpen]=useState(false)
  ...
  useEffect(()=>{                              //concept of click outside to close the dropdown when we click outside the dropdown
  ...
  <motion.div
    ...
    className="fixed top-0 left-0 h-full w-[75%] sm:w-[60%] z-9999 bg-linear-to-b from-green-800/90 via-green-700/80 to-green-900/90 backdrop-blur-xl border-r border-green-400/20 shadow-[0_0_50px_-10px_rgba(255,100,0,0.3)] flex flex-col p-6 text-white"
  >

    <div className="flex justify-between items-center mb-2">
      <h1 className="text-2xl text-white tracking-wide font-semibold"> Admin Panel </h1>
      <button
 ...
 document.body
  ...
  return (
    <div className="w-[95%] fixed top-4 left-1/2 -translate-x-1/2 bg-linear-to-r from-green-500 to-green-700 rounded-2xl shadow-lg shadow-black/30 flex justify-between items-center h-20 px-4 md:px-8 z-[9999] ">
    ...
    Snapcart
      ...
      {user.role=="user" && 
      <form onSubmit={handleSearch} className="hidden md:flex items-center bg-white rounded-full px-4 py-2 w-1/2 max-w-lg shadow-md">
    ...
    />
      ...
      <div className="flex items-center gap-3 md:gap-6 relative">
        ...
        <AnimatePresence>
          {searchBaxOpen && 
            <motion.div
              ...
              className="fixed top-24 left-1/2 -translate-x-1/2 w-[90%] bg-white rounded-full shadow-lg z-40 flex items-center px-4 py-2 md:hidden">
        ...
        <div className='relative' ref={profileDropDown}>
          <div className='bg-white rounded-full w-11 h-11 flex item-center justify-center overflow-hidden shadow-md hover:scale-105 transition-transform ' onClick={()=>setOpen((prev)=>!prev)}>
          {
            user.image ? <Image src={user?.image} alt="user" fill className="rounded-full object-cover" /> : <User className="w-6 h-6 mt-2"   />
          }
          </div>
