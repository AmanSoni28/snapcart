...
function ViewGrocery() {
  ...
  const handleEdit = async () => {
    ...
    useEffect(()=>{
      ...
      setFilltered(groceries?.filter((g)=>g.name.toLowerCase().includes(q) || g.category.toLowerCase().includes(q)))
    }
    handleSearach()
    },[search])
  ...
  return (
    <div className="pt-4 w-[95%] md:w-[85%] mx-auto pb-20">
      ...
      >
        ...
        <h1 className="text-3xl font-extrabold text-green-700 flex items-center justify-center gap-2">
          ...
          >
            ...
            >
              ...
              <div className="relative aspect-square w-full rounded-lg overflow-hidden mb-4 border border-gray-200 group">
                {imagePreview && (
                  <Image
                    src={imagePreview}
                    alt={editing.name}
                    fill                        //fill the image according to parent size
                ...
                <label
                  htmlFor="imageUpload"
                  className="absolute inset-0 bg-black/40 opacity-60 group-hover:opacity-0 flex items-center justify-center cursor-pointer transition-opacity"
                >
              ...
              <div className="flex justify-end gap-3 mt-6">
                ...
                onClick={handleEdit}>
                   {editLoading ? <Loader2 className="animate-spin" /> : "Edit Grocery"}
                </button>

                <button className="px-4 py-2 rounded-lg bg-red-600 text-white flex items-center gap-2 hover:bg-red-700 transition" onClick={handleDelete}>
                   {deleteLoading ? <Loader2 className="animate-spin" /> : "Delete Grocery"}
                </button>
              </div>
            </motion.div>
          </motion.div>
