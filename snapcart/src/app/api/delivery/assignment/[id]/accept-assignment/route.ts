...
export async function GET(req:NextRequest, {params}: { params: Promise<{ id:string }> }){
  try {
    ...
    if(alreadyAssigned){
        return NextResponse.json(
            {message:"already assigned to another delivery"},
            {status:400}
        )
    }
    ...
    //if deliveryBoy accepted the assignment then remove that assignment from other delivery boys broadcastedTo list
    ...
    return NextResponse.json(
      {message:`accept asssignment error ${error}`},
      {status:500}
    )
  }
}